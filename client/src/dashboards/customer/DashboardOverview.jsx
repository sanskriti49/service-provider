...
import { motion, AnimatePresence } from "framer-motion";
import { Calendar, Clock, Plus, Zap, ChevronRight } from "lucide-react";
import api from "../../api/axiosInstance"; // ✅ uses interceptors — no manual token headers
...
export default function DashboardOverview() {
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				// ✅ axios instance handles Authorization header automatically
				...
				if (dashboardRes.status === "fulfilled") {
					setDashboardStats(dashboardRes.value.data.stats);
				}
			} catch (error) {
				// Global 401 handling is in axios interceptor — only log unexpected errors
				console.error("Failed to load overview data", error);
			} finally {
				setIsLoading(false);
			}
	...
	return (
		...
		>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				...
				<StatCard
					label="Completed"
					value={dashboardStats.total_completed ?? 0}
					color="text-purple-600 bg-purple-50 border-purple-100"
				/>
			</div>
					...
					<div className="grid grid-cols-2 gap-3">
						{QUICK_SERVICES.map((service) => (
							<Link
								key={service.slug}
								to={`/services/${service.slug}`}
								className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-2xl border border-gray-100 bg-white hover:border-violet-200 hover:shadow-md transition-all group"
							>
								<div
									className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${service.color}`}
								>
...
const StatCard = ({ label, value, color }) => (
	<div
		className={`p-5 rounded-2xl border flex flex-col justify-between h-28 ${color}`}
	>
...
const UpcomingCard = ({ booking, isHero, formatTime, formatCurrency }) => (
	...
	>
		<div className="flex justify-between items-start">
			...
			{isHero && <StatusBadge status={booking.status} />}
		</div>
		<div className="mt-4 flex items-center justify-between">
			<div className="text-sm text-gray-500">
				{booking.provider_name || "Provider Assigned"}
...
// ✅ Show the real status instead of always "Confirmed"
const STATUS_STYLES = {
