	...
	try {
		...
		const existingBookings = await client.query(
			`SELECT start_time, end_time, latitude, longitude FROM bookings 
             WHERE provider_id = $1 AND date = $2::date AND status = 'booked'`,
			[provider_id, cleanDate],
		);

		let transitBufferMinutes = 20;
		if (existingBookings.rows.length > 0 && latitude && longitude) {
			for (const existingJob of existingBookings.rows) {
				if (existingJob.latitude && existingJob.longitude) {
					// Check if current requested booking is tight up against an existing job
					const coordinates = [
						[parseFloat(longitude), parseFloat(latitude)],
						[
							parseFloat(existingJob.longitude),
							parseFloat(existingJob.latitude),
						],
					];

					const matrix = await getOSRMDurationMatrix(coordinates);
					if (matrix && matrix[0] && matrix[0][1]) {
						const travelTimeMinutes = matrix[0][1];

						// If travel time is short, give a relaxed grace structural period window,
						// otherwise expand the buffer window requirements
						if (travelTimeMinutes <= 15) {
							transitBufferMinutes = Math.max(transitBufferMinutes, 10);
						} else {
							transitBufferMinutes = Math.max(
		...
		const conflictQ = `
            ...
            WHERE provider_id = $1 AND date = $2::date AND status = 'booked'
        `;
		const allDaySlots = await client.query(conflictQ, [provider_id, cleanDate]);

		for (const slot of allDaySlots.rows) {
			// Convert times to comparable integers/minutes from midnight to cleanly evaluate bounds
			const toMins = (t) => {
			...
			const existStart = toMins(slot.start_time);
			const existEnd = toMins(slot.end_time);

			// If requested booking starts after existing booking ends, ensure travel buffer is satisfied
			if (reqStart >= existEnd && reqStart < existEnd + transitBufferMinutes) {
				await client.query("ROLLBACK");
				return res.status(409).json({
					message: `Booking conflict! Provider requires at least ${transitBufferMinutes} minutes transit window to reach your location.`,
				});
			}

			// Standard block overlapping validation
			if (!(reqEnd <= slot.start_time || reqStart >= slot.end_time)) {
				await client.query("ROLLBACK");
				return res.status(409).json({ message: "Slot already booked!" });
			}
		}
