...
async function createBooking(req, res, next) {
	...
	try {
		...
		// Updated status check to catch active bookings in all confirmed states
		...
		for (const slot of existingBookings.rows) {
			...
			// 2. Targeted Transit Buffer Check (Calculated specifically per adjacent job)
			let requiredBuffer = 20;
			...
			if (latitude && longitude && slot.latitude && slot.longitude) {
				...
				if (isImmediatelyBefore || isImmediatelyAfter) {
					const coordinates = [
						...
						[parseFloat(slot.longitude), parseFloat(slot.latitude)],
					];

					const matrix = await getOSRMDurationMatrix(coordinates);
					if (matrix && matrix[0] && matrix[0][1] != null) {
						const travelTime = matrix[0][1];
						requiredBuffer = travelTime <= 15 ? 10 : travelTime + 10;
					}
				}
			}
			...
			if (reqEnd <= existStart && existStart < reqEnd + requiredBuffer) {
				await client.query("ROLLBACK");
				return res.status(409).json({
					message: `Booking conflict! Provider requires at least ${requiredBuffer} minutes transit window for their next booking.`,
				});
			}
		}
